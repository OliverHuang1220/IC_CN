...
def main(params):
  filename = params['input_json']
  imgs = {filename[filename.rfind('/')+1:filename.rfind('.')].replace('annotations', 'images'):json.load(open(filename, 'r'))}
  tmp = []
  for k in imgs.keys():
    for img in imgs[k]:
...
if __name__ == "__main__":
  ...
  parser = argparse.ArgumentParser()
  # input json
  parser.add_argument('--input_json', default= '/home1/huangqiangHD/imagecaption/data/coco/caption_annotations.json',type=str)
  parser.add_argument('--num_val', default=1000, required=False, type=int, help='number of images to assign to validation data (for CV etc)')
  parser.add_argument('--output_json', default= '/home1/huangqiangHD/IC_CN/data/coco2017/caption_token.json', help='output json file')
